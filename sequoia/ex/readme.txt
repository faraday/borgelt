...
different field separators are used. This file can be processed with:
  sequoia -f ",.;:" -l test2.tab test2.out
...
such a weight, is has to be invoked with the option -w:
  sequoia -w test3.tab test3.out

The files test4.tab to test6.tab are in formats that may be common,
but which cannot be processed directly with the sequoia program.
...
numbers or column names into a format appropriate for the sequoia
...
of the sequoia program are written to standard error.
